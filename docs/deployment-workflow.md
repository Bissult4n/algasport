...
Ветка `codex/verify-preview` предназначена для проверки автоматического preview после push. Её отличие от `main` только в этом пояснении; каталог, дизайн и настройки магазина не изменены. Объединять эту проверочную ветку с `main` не требуется.
